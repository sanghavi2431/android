package com.periodTracker.collapsiblecalendarview.widget
...
import com.periodTracker.collapsiblecalendarview.data.Event
import com.periodTracker.collapsiblecalendarview.view.BounceAnimator
import com.periodTracker.collapsiblecalendarview.view.ExpandIconView
import java.text.DateFormatSymbols
import java.text.SimpleDateFormat
import java.util.*

const val currentDatePattern = "d, MMM yyyy"
class CollapsibleCalendar : UICalendar, View.OnClickListener {
    override fun changeToToday() {
        val calendar = Calendar.getInstance()
        val calenderAdapter =  CalendarAdapter(context, calendar,this);
    ...
    lateinit var getDate: Calendar
    lateinit var startPeriodCalendar: Calendar
    lateinit var endPeriodCalendar: Calendar
    lateinit var endCycleCalendar: Calendar
    lateinit var ovulationStartCalendar: Calendar
    lateinit var ovulationEndCalendar: Calendar
    lateinit var pregnancyStartDate: Calendar
    lateinit var pregnancyEndDate: Calendar
    ...
    protected fun init(context: Context) {
        ...
        val cal = Calendar.getInstance()
        val adapter = CalendarAdapter(context, cal, this)
        setAdapter(adapter)
    ...
    override fun reload() {
        mAdapter?.let { mAdapter ->
            ...
            val currentDateFormat = SimpleDateFormat(currentDatePattern, getCurrentLocale(context))
            dateFormat.timeZone = mAdapter.calendar.timeZone
            mTxtTitle.text = dateFormat.format(mAdapter.calendar.time)
            mTxtCurrentDate.text = currentDateFormat.format(calendar.time)
            ...
            for (i in 0..6) {
                ...
                val dayString = DateFormatSymbols().shortWeekdays[(i + firstDayOfWeek) % 7 + 1]
                txtDayOfWeek.text = dayString.subSequence(0, 1)
    ...
    fun prevMonth() {
        val cal = mAdapter!!.calendar
        previousMonthCount = getCalendarData();
        params.let {
            if (it != null && (Calendar.getInstance()
                    .get(Calendar.YEAR) * 12 + Calendar.getInstance()
                    .get(Calendar.MONTH) - previousMonthCount) >= (cal.get(Calendar.YEAR) * 12 + cal.get(Calendar.MONTH))
            ) {
    ...
    fun nextMonth() {
        ...
        params.let {
            if (it != null && (Calendar.getInstance()
                    .get(Calendar.YEAR) * 12 + Calendar.getInstance()
                    .get(Calendar.MONTH) + 5) < (cal.get(Calendar.YEAR) * 12 + cal.get(Calendar.MONTH))
            ) {
                val myAnim = AnimationUtils.loadAnimation(context, R.anim.bounce)
                val interpolator = BounceAnimator(0.1, 10.0)
                myAnim.setInterpolator(interpolator)
    ...
    fun setCalendarData(data: Int) {
        this.previousMonthCount = data

    }

    fun getCalendarData(): Int {
