...
class PeriodCalendarView : View {
    ...
    private var menstruationDays: MutableList<Int> = mutableListOf()
    private var ovulationDays: MutableList<Int> = mutableListOf()
    private var pregnancyDays: MutableList<Int> = mutableListOf()
    private var currentMonth = 0
    private var currentYear = 0
    private var currentDay = 0
    private var periodDays = 3
    private var nextPeriodStartDay: Int? = null
    private val nextPeriodColor = Color.parseColor("#9EDE97")
    ...
    private fun resetCalendar() {
        ...
        for (number in days) {
            var color = 0
            color = if (nextPeriodStartDay == number) {
                nextPeriodColor
            }
           else if (menstruationDays.indexOf(number) != -1) {
...
//        canvas.drawCircle(cx, cy, (radius + padding - offset) , paint);
    ...
    private fun drawPeriodCircle(canvas: Canvas, type: PERIOD_TYPES) {
       ...
       // canvas.drawBitmap(bg_bitmap, null, Rect(left, top, right, bottom), paint)
        if (bg_bitmap != null) {
            ...
            // Handle the null case, e.g., log an error or use a placeholder
            Log.e("PeriodCalendarView", "Failed to decode pt_background bitmap.")
        }
        Log.e("PeriodCalendarView", "bitmap. $type")
        //Draw background
        ...
        when (type) {
            ...
            PERIOD_TYPES.PERIOD -> icon = `in`.woloo.www.R.drawable.ic_mood_icon_one_normal
        }
        ...
        if (icon !=0 ) {
          ...
          //  canvas.drawBitmap(icon_bitmap, null, Rect(left, top, right, bottom), paint)
            if (icon_bitmap != null) {
                ...
                // Handle the null case, e.g., log an error or use a placeholder
                Log.e("PeriodCalendarView Aarati", "Failed to decode pt_background bitmap.")
            }
        }
    ...
    fun setNextPeriodStart(day: Int) {
        nextPeriodStartDay = day
        invalidate()
        requestLayout()
    }
...
//        31 = - 0.05
   ...
   /* private fun drawMonthTitle(canvas: Canvas) {
        val fontSizeOffset = (width * 0.0150).toInt() //12;
        val fontSize = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_SP,
            fontSizeOffset.toFloat(),
            resources.displayMetrics
        ...
        try {
            val typeface = ResourcesCompat.getFont(
                context, `in`.woloo.www.R.font.gothic
            )
            paint!!.setTypeface(typeface)
        } catch (e: Exception) {
        }
        paint!!.color = ContextCompat.getColor(context, `in`.woloo.www.R.color.pre_menstruation)

        val title = String.format(DateFormatSymbols().months[currentMonth])

        paint!!.getTextBounds(title, 0, title.length, rect)

        val cx = (width - rect.width()) / TWO
        val cy = (height - rect.height()) / TWO
        val topOffset = (height * 0.10).toInt() //65
        canvas.drawText(title, cx.toFloat(), (cy - topOffset).toFloat(), paint!!)
    }*/


    private fun drawMonthTitle(canvas: Canvas) {
        val fontSizeOffset = (width * 0.0150).toInt()
        val fontSize = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_SP,
            fontSizeOffset.toFloat(),
            resources.displayMetrics
        ...
        var title = DateFormatSymbols().shortMonths[currentMonth]

        val startDateStr = mSharedPreference!!.getStoredPreference(
            context,
            SharedPreferencesEnum.PERIOD_STARTING_DATE.getPreferenceKey(),
            ""
        ).toString()

        val cycleLengthStr = mSharedPreference!!.getStoredPreference(
            context,
        ...
        if (startDateStr.isNotEmpty()) {
            try {
                ...
                val cycleLength = cycleLengthStr.toIntOrNull() ?: 28
                val cycleEndDate = startDate.plusDays(cycleLength.toLong())

                val today = LocalDate.now()
                val currentMonthName = DateFormatSymbols().shortMonths[today.monthValue - 1]
                val prevMonthName = DateFormatSymbols().shortMonths[
                    if (today.monthValue - 2 < 0) 11 else today.monthValue - 2
                ]
                val nextMonthName = DateFormatSymbols().shortMonths[
                    if (today.monthValue % 12 == 0) 0 else today.monthValue % 12
                ]

                title = when {
                    // Case 1: Cycle entirely in this month
                    startDate.month == today.month && cycleEndDate.month == today.month -> currentMonthName

                    // Case 2: Start date in previous month, today is current month
                    startDate.monthValue < today.monthValue && cycleEndDate.month == today.month -> "$prevMonthName - $currentMonthName"

                    // Case 3: End spills into next month
                    cycleEndDate.monthValue > today.monthValue -> "$currentMonthName - $nextMonthName"
        ...
        val cx = (width - rect.width()) / 2
        val cy = (height - rect.height()) / 2
        val topOffset = (height * 0.10).toInt()
        canvas.drawText(title, cx.toFloat(), (cy - topOffset).toFloat(), paint!!)
    }
    ...
    private fun drawTitle(canvas: Canvas) {
        ...
        val cx = (width - rect.width()) / TWO
        val cy = (height - rect.height()) / TWO
        val topOffset = (height * 0.05).toInt() //65
        canvas.drawText(title, cx.toFloat(), (cy - topOffset).toFloat(), paint!!)
    }
    ...
    fun setPeriodCycle(menstruation: List<Int>, ovulation: List<Int>, pregnancy: List<Int>) {
        menstruationDays = menstruation.toMutableList()
        ovulationDays = ovulation.toMutableList()
        pregnancyDays = pregnancy.toMutableList()
