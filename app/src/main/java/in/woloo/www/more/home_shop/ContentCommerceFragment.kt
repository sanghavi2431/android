...
import `in`.woloo.www.more.home_shop.ShopBlogsController.OnClickShopBlogViewItems
...
class ContentCommerceFragment : Fragment(), NearByWolooAndOfferCountView,
    OnClickShopBlogViewItems {
    @BindView(R.id.epoxyRecyclerShop)
   ...
   */
   ...
   /* @BindView(R.id.overlay_layout_c_c)
    lateinit var overlay_layout :FrameLayout*/
    ...
    private var blogController: ShopBlogsController? = null
    ...
    override fun onCreateView(
        ...
        val rootView = inflater.inflate(R.layout.fragment_content_commerce, container, false)
...
//        nearByWolooAndOfferCountPresenter.getBlogs(category, page);
        blogViewModel!!.getBlogsForShopByCategory(category, page, AppConstants.BLOG_CAT_TYPE_ONE)
...
//                    nearByWolooAndOfferCountPresenter.getBlogs(category, page);
                        previousPage =
                            if (WolooDashboard.Companion.blogType.matches("0".toRegex())) {
                                blogViewModel!!.getBlogsForShopByCategory(
                                    category,
                                    page,
                                    AppConstants.BLOG_CAT_TYPE_ONE
                                )
                                page
                            } else {
                                blogViewModel!!.getBlogsForShopByCategory(
        ...
        blogController = ShopBlogsController(requireActivity(), this)
        epoxyRecyclerView!!.setController(blogController!!)




      /*  if (WolooDashboard.mSharedPreference!!.getStoredBooleanPreference(
                requireActivity(),
        ...
        overlay_layout.setOnClickListener { v: View? ->
            WolooDashboard.mSharedPreference!!.setStoredBooleanPreference(
                requireActivity(),
                SharedPreferencesEnum.FIRST_TIME_APP_LAUNCHED.getPreferenceKey(),
                false
            )
    ...
    private fun setLiveData() {
        homeViewModel!!.observeNearByWolooAndOfferCount().observe(viewLifecycleOwner,
            Observer<BaseResponse<NearByWolooAndOfferCountResponse.Data>?> { response ->
      ...
      /*  blogViewModel!!.observeCtaFavourite().observe(viewLifecycleOwner,
            Observer<ApiResponseData<BaseResponse<JSONObject>>> { response ->
                if (response != null && response.data != null) {
                    onFavouriteABlog()
                }
            })*/
...
//            nearByWolooAndOfferCountPresenter.addBlogReadPoints(currentBlog);
    ...
    fun offerPopUpwindow() {
        ...
        dialog.window!!.setBackgroundDrawable(ColorDrawable(requireContext().resources.getColor(R.color.transparent)))
    ...
    private fun navigateToWebView() {
        if (currentBlog != null) {
            val webViewFragment = newInstance(
                currentBlog!!.title, currentBlog!!.shortLink, "TrendBlogFragment"
            )
        ...
        if (requireActivity() is WolooDashboard) {
            ...
            (requireActivity() as WolooDashboard).changeIcon(
                (requireActivity() as WolooDashboard).nav_view!!.menu
                    .findItem(R.id.navigation_inviteFriend)
            )
        }
    }
    ...
    override fun onSelectCategory(position: Int) {
        ...
        blogs.clear()
        //        nearByWolooAndOfferCountPresenter.getBlogs(category, page);
        // blogViewModel.getBlogsForShopByCategory(category, page);
        if (WolooDashboard.Companion.blogType.matches("0".toRegex())) {
            blogViewModel!!.getBlogsForShopByCategory(category, page, AppConstants.BLOG_CAT_TYPE_ONE)
        } else {
            blogViewModel!!.getBlogsForShopByCategory(category, page, AppConstants.BLOG_CAT_TYPE_ONE)
            ...
            Response.Listener<String?> { response ->
                try {
                    ...
                    val fragmentManager: FragmentManager =
                        (context as WolooDashboard?)!!.supportFragmentManager
                    val fragmentTransaction = fragmentManager.beginTransaction()
                    fragmentTransaction.replace(R.id.frm_contant, myFragment, ContentCommerceFragment.TAG)
                    fragmentTransaction.addToBackStack(null)
                    fragmentTransaction.commit()
                } catch (e: Exception) {
    ...
    companion object {
        const val TAG = "TrendingBlogFragment"
    }
}