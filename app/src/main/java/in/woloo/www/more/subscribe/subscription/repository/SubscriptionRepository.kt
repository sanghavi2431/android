...
class SubscriptionRepository : BaseRepository() {
    ...
    fun submitGiftCardSubscriptionPurchase(
        request: SubmitGiftCardSubscriptionPurchaseRequest,
        webserviceCallback: WebserviceCallback<ApiResponseData<BaseResponse<InitSubscriptionResponse>>>
    ) {
        if (NetworkUtils.isInternetAvailable(WolooApplication.instance!!)) {
            try {
                val call: Call<BaseResponse<InitSubscriptionResponse>> =
                    apiService.submitGiftCardSubscriptionPurchase(request)
                val callback: ApiServiceCallback<BaseResponse<InitSubscriptionResponse>> =
                    ApiServiceCallback(webserviceCallback)
                call.enqueue(callback)
            } catch (e: Exception){
    ...
    fun getPurchaseNow(
        request: PurchaseNowRequest,
        webserviceCallback: WebserviceCallback<ApiResponseData<BaseResponse<PurchaseNowResponse>>>
    ) {
        if (NetworkUtils.isInternetAvailable(WolooApplication.instance!!)) {
            try {
                val call: Call<BaseResponse<PurchaseNowResponse>> =
                    apiService.getPurchaseNow(request)
                val callback: ApiServiceCallback<BaseResponse<PurchaseNowResponse>> =
                    ApiServiceCallback(webserviceCallback)
                call.enqueue(callback)
            } catch (e: Exception){
                CommonUtils.printStackTrace(e)
            ...
            val data = ApiResponseData<BaseResponse<PurchaseNowResponse>>()
            data.status = ApiResponseData.API_NO_NETWORK
            webserviceCallback.onWebResponse(data)
        }
    }
