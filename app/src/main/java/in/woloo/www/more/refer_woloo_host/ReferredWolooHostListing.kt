...
class ReferredWolooHostListing : AppCompatActivity() {
    ...
    private fun initView() {
      //  tvTitle!!.text = "Refer a Woloo Host"
        ...
        gifImageView?.let {
            Glide.with(this)
                .load(R.drawable.refer_host_gif) // your gif in res/drawable
                .into(it)
        }
    }
            ...
            if (referredWolooListResponse != null && referredWolooListResponse.data != null) {
                if (referredWolooListResponse.data!!.size > 0) {
                    ...
                    for (i in referredWolooListResponse.data!!.indices) {
                       // if (referredWolooListResponse.data!!.get(i).status == 0)
                        if(referredWolooListResponse.data!!.get(i).status == 1)
                            underReviewCnt++
                    }
                    if (referredWolooListResponse.data!!.size >= 3) referWolooImv!!.visibility =
