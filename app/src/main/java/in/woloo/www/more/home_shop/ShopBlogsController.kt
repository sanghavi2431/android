...
class ShopBlogsController (
    ...
    private val onClickShopBlogViewItems: OnClickShopBlogViewItems
) :
    EpoxyController() {
    ...
    @AutoModel
    lateinit var noBlogsModel: NoShopsBlogsModel_

    @AutoModel
    lateinit var blogTextModel: ShopsHeaderModel_
    ...
    override fun buildModels() {
        ...
        if (categories != null && categories!!.size > 0) {
            val categoryModels: MutableList<ShopCategoryModel_> = ArrayList<ShopCategoryModel_>()
            ...
            while (i < categoriesSize) {
                ...
                val categoryModel_ = ShopCategoryModel_()
                categoryModel_.id(category.id!!.toLong())
                    .onClickBlogViewItems(onClickShopBlogViewItems)
                    .category(category)
                    .itemPosition(i)
            ...
            blogTextModel.addTo(this)
            CustomCarouselModel_().id("blog_categories") // .padding(Carousel.Padding.dp(16, 4, 16, 16, 8))
                .models(categoryModels)
                .addTo(this)

          //  setBackgroundForCategoryCarousel()

        }

            blogs?.let { blogList ->
                for (i in blogList.indices) {
                    var sizedArray : Int  = (blogs?.get(i)!!.mainImage!!.size) + (blogs?.get(i)!!.videoUrl!!.size)
        ...
        Log.d("Blogs are" , blogs.toString())



      //  noBlogsModel = NoShopsBlogsModel_()
        noBlogsModel.addIf(blogs?.size == 0, this)
        }


    private fun setBackgroundForCategoryCarousel() {
        val epoxyRecyclerView = EpoxyRecyclerView(context)
        epoxyRecyclerView.setBackgroundResource(R.drawable.yello_rectangle_shape)
    }

    public interface OnClickShopBlogViewItems {
