    ...
    override fun onBindViewHolder(holder: BlogVerticaViewlHolder, position: Int) {
        ...
        try {
            ...
            try {
                ...
                if (!blog.mainImage?.get(0)?.contains("mp4")!!) {
                    ImageUtil.loadImageBlogs(
                        context,
                        holder.ivBlogItem!!,
                        AppConstants.CHANGED_BLOGS_IMAGE_URL + blog.mainImage?.get(0)
                    )
                } else {
                    ImageUtil.loadImageBlogs(
                        context,
                        holder.ivBlogItem!!,
                        AppConstants.CHANGED_BLOGS_IMAGE_URL + blog.mainImage?.get(1)
                    )
                }
            }catch (ex:Exception)
            {
            ...
            holder.tvBlogUpdatedTime!!.text =
                TimeAgoUtils.getTimeAgo(CommonUtils.getTimeAgo(blog.updatedAt!!))
