...
@EpoxyModelClass(layout = R.layout.home_shop_blog_four_adapter)
abstract class ShopBlogsItemModelFour  : EpoxyModelWithHolder<ShopBlogsItemModelFour.Holder>() {
    @EpoxyAttribute
    ...
    @EpoxyAttribute
    open var isBlogRead = 0
    override fun bind(holder: ShopBlogsItemModelFour.Holder) {
        try {
            val description = blog!!.title

            // Show truncated description (1.5 lines)
            val visibleDescription =
                if (description!!.length > 100) description.substring(
                    0,
                    100
                ) + "..." else description
            ...
            holder.tvShopNow?.setOnClickListener {
                var shopLinkId = blog!!.isMapToShop
            ...
            holder.tvBlogTitle?.setOnClickListener { v: View? ->
                if (holder.tvBlogTitle!!.text.toString().contains("SeeMore...")) {
                    ...
                    val fullDescription =
                        description // Use the original full description
                }
            }
            ImageUtil.loadImageBlogs(
                holder.ivBlogImgFourOne?.context!!,
                holder.ivBlogImgFourOne!!,
                BuildConfig.BASE_URL + AppConstants.DEFAULT_BASE_URL_FOR_BLOG_IMAGE + (blog!!.mainImage?.get(
                    0
                ))
            )
            ImageUtil.loadImageBlogs(
                holder.ivBlogImgFourTwo?.context!!,
                holder.ivBlogImgFourTwo!!,
                BuildConfig.BASE_URL + AppConstants.DEFAULT_BASE_URL_FOR_BLOG_IMAGE + (blog!!.mainImage?.get(
                    1
                ))
            )
            ImageUtil.loadImageBlogs(
                holder.ivBlogImgFourThree?.context!!,
                holder.ivBlogImgFourThree!!,
                BuildConfig.BASE_URL + AppConstants.DEFAULT_BASE_URL_FOR_BLOG_IMAGE + (blog!!.mainImage?.get(
                    2
                ))
            )
            ImageUtil.loadImageBlogs(
                holder.ivBlogImgFourFour?.context!!,
                holder.ivBlogImgFourFour!!,
                BuildConfig.BASE_URL + AppConstants.DEFAULT_BASE_URL_FOR_BLOG_IMAGE + (blog!!.mainImage?.get(
                    3
                ))
            )
            ...
            if (isBlogRead > 0) {
                holder.rlBlogPoints?.backgroundTintList =
                    ContextCompat.getColorStateList(holder.rlBlogPoints!!.context, R.color.gray)
            } else {
                holder.rlBlogPoints?.backgroundTintList =
                    ContextCompat.getColorStateList(
                        holder.rlBlogPoints!!.context,
                        R.color.sheildcolor
                    )
            }
            holder.ivBlogImgFourOne?.setOnClickListener { view: View? ->
                onClickBlogViewItems!!.onClickBlogItem(
                    blog!!,
                    itemPosition
                )
            }
            holder.ivBlogImgFourTwo?.setOnClickListener { view: View? ->
                onClickBlogViewItems!!.onClickBlogItem(
                    blog!!,
                    itemPosition
                )
            }
            holder.ivBlogImgFourThree?.setOnClickListener { view: View? ->
                onClickBlogViewItems!!.onClickBlogItem(
                    blog!!,
                    itemPosition
                )
            }
            holder.ivBlogImgFourFour?.setOnClickListener { view: View? ->
                onClickBlogViewItems!!.onClickBlogItem(
                    blog!!,
                    itemPosition
                )
    ...
    class Holder : BaseEpoxyHolder() {
        @JvmField
        @BindView(R.id.blogImgFourOne)
        var ivBlogImgFourOne: ImageView?= null

        @JvmField
        @BindView(R.id.blogImgFourTwo)
        var ivBlogImgFourTwo: ImageView?= null

        @JvmField
        @BindView(R.id.blogImgFourThree)
        var ivBlogImgFourThree: ImageView?= null

        @JvmField
        @BindView(R.id.blogImgFourFour)
        var ivBlogImgFourFour: ImageView?= null
