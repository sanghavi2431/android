...
@EpoxyModelClass(layout = R.layout.home_shop_blog_adapter)
abstract class ShopBlogsItemModel  : EpoxyModelWithHolder<ShopBlogsItemModel.Holder>() {
    @EpoxyAttribute
    ...
    override fun bind(holder: ShopBlogsItemModel.Holder) {
        try {
            ...
            holder.tvBlogTitle?.setOnClickListener { v: View? ->
                if (!isExpanded) {
                   ...
                   holder.tvBlogTitle!!.ellipsize = TextUtils.TruncateAt.END
isExpanded = true
                }
                else{
                    holder.tvBlogTitle!!.maxLines = 2
                    isExpanded = false
                }
            }
            if(blog!!.videoUrl.isNullOrEmpty()) {
                ImageUtil.loadImageBlogs(
                    holder.ivBlogItem?.context!!,
                    holder.ivBlogItem!!,
