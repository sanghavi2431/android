...
class SubscriptionViewModel : BaseViewModel() {
    ...
    var submitGiftCardSubscriptionPurchaseResponse : EventLiveData<BaseResponse<InitSubscriptionResponse>> = EventLiveData()
    var purchaseNowSubscription : EventLiveData<BaseResponse<PurchaseNowResponse>> = EventLiveData()
    ...
    fun submitGiftCardSubscriptionPurchase(request : SubmitGiftCardSubscriptionPurchaseRequest){
        updateProgress(true)
        subscriptionRepository.submitGiftCardSubscriptionPurchase(request, object :
            WebserviceCallback<ApiResponseData<BaseResponse<InitSubscriptionResponse>>> {
            override fun onWebResponse(data: ApiResponseData<BaseResponse<InitSubscriptionResponse>>) {
                updateProgress(false)
                if(data.status == ApiResponseData.API_SUCCESS){
                    submitGiftCardSubscriptionPurchaseResponse.value = data.data
                }else{
                    WolooApplication.errorMessage = data.message
                    submitGiftCardSubscriptionPurchaseResponse.value = data.data
                    notifyNetworkError(data)
    ...
    fun getPurchaseNow(request : PurchaseNowRequest){
        updateProgress(true)
        subscriptionRepository.getPurchaseNow(request, object :
            WebserviceCallback<ApiResponseData<BaseResponse<PurchaseNowResponse>>> {
            override fun onWebResponse(data: ApiResponseData<BaseResponse<PurchaseNowResponse>>) {
                updateProgress(false)
                if(data.status == ApiResponseData.API_SUCCESS){
                    purchaseNowSubscription.value = data.data
                }else{
                    WolooApplication.errorMessage = data.message
                    purchaseNowSubscription.value = data.data
                    notifyNetworkError(data)
    ...
    fun observeSubmitGiftCardSubscriptionPurchaseResponse() : EventLiveData<BaseResponse<InitSubscriptionResponse>> {
        return submitGiftCardSubscriptionPurchaseResponse
    }

    fun observePurchaseNow() : EventLiveData<BaseResponse<PurchaseNowResponse>> {
        return purchaseNowSubscription
    }
