...
class ServicesProductListAdapter (private val context: FragmentActivity,
                                  private var productList: ArrayList<ProductListData>,
                                  private val fragmentManager: FragmentManager,
                                  private val servicesViewModel: ServiceViewModel) : RecyclerView.Adapter<ServicesProductListAdapter.ViewHolder>() {
        ...
        try {
           ...
           /* if(product.variants!!.get(0).inventoryQuantity == 0)
            ...
            holder.binding.notifyCartView.setOnClickListener {
                ...
                servicesViewModel.getNotifyUserForProduct(request)
...
// Restore after 10–15 seconds
                ...
                Logger.i("Aarati Product details" , product.id.toString() + " " + productList[position] + " " + productList[position].id )
                try{
                    val bottomSheetFragment = DateTimeBottomSheetFragment.newInstance(variantId.toString() ,productList[position] , "PRODUCT_DETAILS")
