...
class SearchFromServicesActivity  : AppCompatActivity() {

    lateinit var binding : FragmentSearchFromStoreBinding
    private var storeViewModel: ServiceViewModel? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        storeViewModel = ViewModelProvider(this).get<ServiceViewModel>(
    ...
    Logger.i("Aarati Store", "setLiveData ${itemList[i] + itemList[i]}")
        ...
        binding.viewAllProductsText.setOnClickListener{
           // val position = (itemList.size) - 1
                ...
                val intent = Intent(this, ServicesProductsListingActivity::class.java)
            ...
            response?.let {
                if (!it.products.isNullOrEmpty()) {
                    ...
                    if (!it.products.isNullOrEmpty()) {
                        ...
                        cartList?.let { productsCollectionsCustomeAdapter?.updateCart(it) }
                        if(it.products!= null)
                        binding.viewAllProductsText.visibility = ViewGroup.GONE
                    }
                    else{
                        Toast.makeText(this , "No Product Found" , Toast.LENGTH_SHORT).show()
                    }
