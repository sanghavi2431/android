...
class SharedPrefClient {
    ...
    fun setJsonRequest(key: String , jsonObject: JSONObject) {
            sharedPreferences.edit().putString(key, jsonObject.toString()).apply()
    }

    fun getJsonRequest(key: String): JSONObject? {
        return sharedPreferences.getString(key, "{}")?.let { JSONObject(it) }
    ...
    fun removeArrayList(key: String) {
        sharedPreferences.edit().remove(key).apply()
    }
