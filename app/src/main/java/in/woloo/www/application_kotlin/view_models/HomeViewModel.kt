...
import `in`.woloo.www.application_kotlin.presentation.fragments.web_view.WebViewFragment.Companion
...
class HomeViewModel : BaseViewModel() {
    ...
    private val mSearchNearbyWoloosForSearch: EventLiveData<BaseResponse<ArrayList<NearByStoreResponse.Data>>> = EventLiveData()
    private val mSearchNearbyWoloos: EventLiveData<BaseResponse<NearByStoreResultsWrapper>> = EventLiveData()
    ...
    private val mAboutUs: EventLiveData<BaseResponse<WebViewResponse>> = EventLiveData()
    private val mtermsAndConditions: EventLiveData<BaseResponse<WebViewResponse>> = EventLiveData()
    ...
    /*fun searchNearbyWoloos(request: SearchWolooRequest) {
        Log.d(TAG, "getNearbyWoloos:$request")
        //updateProgress(true)
        mHomeRepository.searchNearbyWoloos(request, object :
            WebserviceCallback<ApiResponseData<BaseResponse<ArrayList<NearByStoreResponse.Data>>>> {
            override fun onWebResponse(data: ApiResponseData<BaseResponse<ArrayList<NearByStoreResponse.Data>>>) {
                //updateProgress(false)
                if (data.status == ApiResponseData.API_SUCCESS) {
    ...
    fun searchNearbyWoloos(request: SearchWolooRequest) {
        mHomeRepository.searchNearbyWoloos(request, object :
            WebserviceCallback<ApiResponseData<BaseResponse<NearByStoreResultsWrapper>>> {
            override fun onWebResponse(data: ApiResponseData<BaseResponse<NearByStoreResultsWrapper>>) {
                if (data.status == ApiResponseData.API_SUCCESS) {
                    mSearchNearbyWoloos.value = data.data
                } else {
                    mSearchNearbyWoloos.value = data.data
                    notifyNetworkError(data)
                }
    ...
    fun searchNearbyWoloosForSearch(request: SearchWolooRequest) {
        mHomeRepository.searchNearbyWoloosForSearch(request, object :
            WebserviceCallback<ApiResponseData<BaseResponse<ArrayList<NearByStoreResponse.Data>>>> {
            override fun onWebResponse(data: ApiResponseData<BaseResponse<ArrayList<NearByStoreResponse.Data>>>) {
                //updateProgress(false)
                if (data.status == ApiResponseData.API_SUCCESS) {
                    mSearchNearbyWoloosForSearch.value = data.data
                } else {
                    mSearchNearbyWoloosForSearch.value = data.data
                    notifyNetworkError(data)
                }
    ...
    fun observeSearchNearByWolooForSearch(): EventLiveData<BaseResponse<ArrayList<NearByStoreResponse.Data>>> {
        return mSearchNearbyWoloosForSearch
    }

    fun observeSearchNearByWoloo(): EventLiveData<BaseResponse<NearByStoreResultsWrapper>> {
        return mSearchNearbyWoloos
    }
    ...
    fun observeAboutUsStatus(): MutableLiveData<BaseResponse<WebViewResponse>> {
        return mAboutUs
    }

    fun getAboutUsStatus(){
        mHomeRepository.getAboutUsStatus(object :
            WebserviceCallback<ApiResponseData<BaseResponse<WebViewResponse>>> {
            override fun onWebResponse(data: ApiResponseData<BaseResponse<WebViewResponse>>) {
                if (data.status == ApiResponseData.API_SUCCESS) {
                    Logger.i(WebViewFragment.TAG, "setLiveData ${data}")
                    Logger.i(WebViewFragment.TAG, "setLiveData ${data.data}")
                    mAboutUs.value = data.data
                } else {
                    WolooApplication.errorMessage = data.message
                    mAboutUs.value = data.data
                    notifyNetworkError(data)
    ...
    fun observeTermsStatus(): MutableLiveData<BaseResponse<WebViewResponse>> {
        return mtermsAndConditions
    }

    fun getTermsStatus(){
        mHomeRepository.getTermsStatus(object :
            WebserviceCallback<ApiResponseData<BaseResponse<WebViewResponse>>> {
            override fun onWebResponse(data: ApiResponseData<BaseResponse<WebViewResponse>>) {
                if (data.status == ApiResponseData.API_SUCCESS) {
                    mtermsAndConditions.value = data.data
                } else {
                    WolooApplication.errorMessage = data.message
                    mtermsAndConditions.value = data.data
                    notifyNetworkError(data)
