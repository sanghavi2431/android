...
class HomeRepository : BaseRepository() {
   ...
   /* fun searchNearbyWoloos(
        request: SearchWolooRequest,
        webserviceCallback: WebserviceCallback<ApiResponseData<BaseResponse<ArrayList<NearByStoreResponse.Data>>>>
    ) {
        if (NetworkUtils.isInternetAvailable(WolooApplication.instance!!)) {
            try {
                val call: Call<BaseResponse<ArrayList<NearByStoreResponse.Data>>> =
                    apiService.searchNearbyWoloos(request)
                val callback: ApiServiceCallback<BaseResponse<ArrayList<NearByStoreResponse.Data>>> =
                    ApiServiceCallback(webserviceCallback)
                call.enqueue(callback)
            } catch (e: Exception){
    ...
    fun searchNearbyWoloos(
        request: SearchWolooRequest,
        webserviceCallback: WebserviceCallback<ApiResponseData<BaseResponse<NearByStoreResultsWrapper>>>
    )
    {
        if (NetworkUtils.isInternetAvailable(WolooApplication.instance!!)) {
            try {
                val call: Call<BaseResponse<NearByStoreResultsWrapper>> =
                    apiService.searchNearbyWoloos(request)
                val callback: ApiServiceCallback<BaseResponse<NearByStoreResultsWrapper>> =
                    ApiServiceCallback(webserviceCallback)
                call.enqueue(callback)
            } catch (e: Exception) {
                CommonUtils.printStackTrace(e)
            ...
            val data = ApiResponseData<BaseResponse<NearByStoreResultsWrapper>>()
            data.status = ApiResponseData.API_NO_NETWORK
            webserviceCallback.onWebResponse(data)
        }
    }

    fun searchNearbyWoloosForSearch(
        request: SearchWolooRequest,
        webserviceCallback: WebserviceCallback<ApiResponseData<BaseResponse<ArrayList<NearByStoreResponse.Data>>>>
    )
    {
        if (NetworkUtils.isInternetAvailable(WolooApplication.instance!!)) {
            try {
                val call: Call<BaseResponse<ArrayList<NearByStoreResponse.Data>>> =
                    apiService.searchNearbyWoloosForSearch(request);
                val callback: ApiServiceCallback<BaseResponse<ArrayList<NearByStoreResponse.Data>>> =
                    ApiServiceCallback(webserviceCallback)
                call.enqueue(callback)
            } catch (e: Exception) {
    ...
    fun getAboutUsStatus(
        webserviceCallback: WebserviceCallback<ApiResponseData<BaseResponse<WebViewResponse>>>
    ) {
        if (NetworkUtils.isInternetAvailable(WolooApplication.instance!!)) {
            try {
                val call: Call<BaseResponse<WebViewResponse>> =
                    apiService.getAboutUs()
                val callback: ApiServiceCallback<BaseResponse<WebViewResponse>> =
                    ApiServiceCallback(webserviceCallback)
                call.enqueue(callback)
            } catch (e: Exception) {
                CommonUtils.printStackTrace(e)
            ...
            val data = ApiResponseData<BaseResponse<WebViewResponse>>()
            data.status = ApiResponseData.API_NO_NETWORK
            webserviceCallback.onWebResponse(data)
        }
    }
