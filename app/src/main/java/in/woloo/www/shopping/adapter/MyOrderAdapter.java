...
public class MyOrderAdapter extends ArrayAdapter {
    ...
    public View getView(int position, final View convertView, ViewGroup parent) {
        ...
        holder.priceTextView.setText("Rs. "+item.getPrice()+" X "+item.getQty());
        holder.order_id.setText("Order Id: "+item.getOrder_id());
        holder.status.setText("Order Status: "+item.getStatus());
        holder.amount.setText("Amount: "+item.getAmount());
        holder.date_time.setText(item.getDate_time());

        if(item.getStatus().equalsIgnoreCase("Return"))
        {
        ...
        if(item.getCan_return().equalsIgnoreCase("1")) {
                ...
                confirm_button.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        if(confirm_checkbox.isChecked()) {
                            ...
                            returnProduct(item.getId(), item.getPro_id());

                            item.setStatus("Return");
                            item.setCan_return("0");
