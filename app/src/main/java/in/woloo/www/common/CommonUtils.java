...
import in.woloo.www.application_kotlin.api_classes.CustomVolleyRequest;
...
public class CommonUtils implements Serializable{
    ...
    public static JSONObject getCustomLocale(Context context, String APP_TYPE) {
        ...
        if (context == null) {
            context = WolooApplication.Companion.getInstance();
        }
        ...
        try {
            if (mStrLocale != null) {
                ...
                try {
                    ...
                    if (versions.countryCode.equals("-")) {
...
//                    jsonObject.put(JSONTagConstant.COUNTRY, "GB");
    ...
    public void sendAPIErrorLogToServer(final Context context, String screenName, String requestName, String inputRequest, String outputResponse) {
        try {
            ...
            RequestQueue queue = VolleySingleton.getInstance(context).requestQueue;
            // showProgressDialog();
...
//        DecimalFormat f = new DecimalFormat("##.00");
    ...
    public static boolean isSubscriptionExpired(String expiryDate){
        Logger.i("AARATI TAG", "isSubscriptionExpired");
        ...
        try {
            ...
            if(currentDate.after(date)){
                Logger.i("AARATI TAG", date +" isSubscriptionExpired true");
                return true;
            }
            Logger.i("AARATI TAG", date +"isSubscriptionExpired false");
            return false;
        } catch (ParseException ex) {
            Logger.v("Exception", ex.getLocalizedMessage());
            return true;
...
//            SimpleDateFormat sdf1 = new SimpleDateFormat("yyyy-MM-dd");
    ...
    public static void navigateToPerUseFlow(Context mContext, String subscriptionId, boolean isEmail, String mobile , String wolooId)   {
        try {
            Logger.i(TAG, "navigateToRazorPayFlow");
            Intent intent = new Intent(mContext, RazorPayActivity.class);
            intent.putExtra(AppConstants.ORDER_ID, subscriptionId);
            intent.putExtra(AppConstants.IS_EMAIL, isEmail);
            intent.putExtra(AppConstants.MOBILE,mobile);
            intent.putExtra(AppConstants.PAYPERUSE , "PAYPERUSE");
            intent.putExtra(AppConstants.WOLOO_ID_Pay , wolooId);
            mContext.startActivity(intent);
        }catch (Exception ex){
            CommonUtils.printStackTrace(ex);
        }
...
//             CommonUtils.printStackTrace(e);
    ...
    public static void calldeeplink(Context context,Dialog mProgressBar,String subject, String shareBody, String longUrl, Boolean isWhatsapp) {
        ...
        Logger.i("aarati suffix", suffix.toString());
        ...
        try {
            ...
            Logger.i("aarati suffix json string", json_string.toString());
        } catch (Exception e) {
             CommonUtils.printStackTrace(e);
        }
                ...
                new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        try {
                            ...
                            Log("aarati share message " + shareMessage);
